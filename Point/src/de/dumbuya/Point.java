package de.dumbuya;
/*
You have to represent a point in 2D space. A class with the name Point. The class needs two fields (instance variables) with name x and y of type int.
...
public class Point {
    ...
    public double distance() {
        return Math.sqrt(Math.abs(getX() * getX()) + Math.abs(getY() * getY()));
    }

    public double distance(int x, int y) {
        return Math.sqrt((Math.abs(x-this.getX()) * Math.abs(x-this.getX()))+(Math.abs(y-this.getY()) * Math.abs(y-this.getY())));
    }
    public double distance(Point point) {
        return Math.sqrt((Math.abs(getX()-point.getX()) * Math.abs(getX()-point.getX()))+(Math.abs(getY()-point.getY()) * Math.abs(getY()-point.getY())));
    }
}